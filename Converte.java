...
public class Converte {
	...
	public static void main(String[] args) throws IOException {
		...
		/*
		 * Maquina de entrada deve estar contido em um arquivoe obdecer a este
		 * padrão obs: Sem tabulação;
		 * 
		 * ( {q0,q1,q2}, {a,b,c}, { (q0,a)->{q0,q1,q2}, (q1,b)->{q1},
		 * (q2,c)->{q2}, (q2,.)->{q1} }, q0, {q1} )
		 * 
		 * 
		 * Obs: para fins praticos a variavel " maximoDeEstadoAFD " foi definida
		 * em 30sendo que teoricamnte ela deveria ser definda como
		 * "2^quantidade de estados"
		 */

		FileReader arq = new FileReader(
				"/home/davidg/Documentos/ProgramasJava/ProgramasComEclip/Converte/src/desc af2.txt");
		...
		int[][][] tabelaAFD;
		/*
		 * Numero de linha representa os estados Numero de colunas representa
		 * alfabeto ex: q1 = 1 , q2 = 2... a=1, b=2 ...
		 */

		String readLine = lerArq.readLine(); // Ler linha 1 e descartar '('
		readLine = lerArq.readLine(); // Ler linha 2// linha do conjunto de
										// estados {q1,q2,q3..},

		String[] removeLinhaBranca = readLine.split("}");// Separar conjunto
		char[] vetorChar = removeLinhaBranca[0].toCharArray();// Converter
																// String em um
																// vetor de char

		for (int i = 0; i < vetorChar.length; i++)
			// Contar quantos estados estão no conjunto
			if (vetorChar[i] == 'q')
				quantidadeEstados++;

		int tamanhoMaxDosConjuntos = quantidadeEstados + 1;
		/*
		 * Garante que o ultimo elemento do conjunto seja invalido(999) para
		 * garantir condições de parada ex: while( tabelaAFNDLambda[ linha ][
		 * coluna ][ elementoDoConjunto ] != 999 )
		 */

		readLine = lerArq.readLine(); // Ler linha 3 // conjunto de alfabeto ex:
										// {a, b, c}
		removeLinhaBranca = readLine.split(" ");// Separar conjunto
		vetorChar = removeLinhaBranca[0].toCharArray();// Converter String em um
														// vetor de char

		for (int i = 0; i < vetorChar.length; i++)
			// Contar quantos simbolos estão no conjunto
			if (vetorChar[i] == ',')

				tamanhoDoAlfabeto++;// Correção sempre uma virgula mais ex{a,b}
									// 2 simbolos 1 virgula

		int colunaLambda = tamanhoDoAlfabeto;

		int quantidadeDeColunasTabelaAFNDLambda = tamanhoDoAlfabeto + 2;// coluana lambda e fech  lambda
																		
		int colunaFechoLambda = colunaLambda + 1;

		tabelaAFNDLambda = new int[tamanhoMaxDosConjuntos][quantidadeDeColunasTabelaAFNDLambda][tamanhoMaxDosConjuntos];// O tamanho max do cunjuntos(tercei vetor  da tabela) é o o numero de estados
		...
		while (vetorChar[0] != '}') {// Loop ate o fim do conjunto de transições
			...
			int posConjunto = 0;// Conta a posição onde serar inserido os
								// estados
			...
			if (caracterColuna == '.') {// Coluna de transição lambda é a ultima
				 ...
				 * "colunaLambda + 48 refere-se a um numero da tabela da tabela
				 * ascii Ex: 50(ascii) = 2(coluna) Obs: O metodo
				 * "getNumericValue" recebe um numero decimal da tabela ascii
				 */
				caracterColuna = colunaLambda + 48;

			} else {
				caracterColuna = caracterColuna - 49;// Converte letra para
														// numero ex: a = 0, b =
														// 1 ...//Referente a
														// coluna
			}
			...
			do {
				...
				if (i < vetorChar.length - 1)// Proteção para i não ultrapassar
												// o tamanho do vetorChar
					i++;
			...
			} while (vetorChar[i] == 'q');// Marca o fim do conjunto dessa
											// transição
		...
		}// Fim do loop da laitura de todas as transiçoes do conjunto de
			// transiçoes

		for (int estado = 0; estado < quantidadeEstados; estado++)
			// Para cada estado(linha da tabela) forma seu Fecho Lambda
		...
		for (int linha = 0; linha < quantidadeEstados; linha++) {
			/*
			 * Este conjunto de instrução realiza a montagem da tabaela AFND.É
			 * dividido e duas partes:
			 * 
			 * 1 Parte: Realiza a inserção que eu chamo de inserção em "L". Para
			 * cada elemento(estado) do conjunto pertencente
			 * tabelaAFNDLambda[linha][coluna][...] va ate ao seu Conjunto
			 * Fecholambda e insira na tabelaAFND[linha][coluna][...]
			 */
			...
			for (int coluna = 0; coluna < tamanhoDoAlfabeto; coluna++) {
				...
				while (tabelaAFNDLambda[linha][coluna][elementoDoConjunto] != 999) {// Loop em todos os elentos do conjunto
					...
					/*
					 * Erro em estrutura de if sem chaves "{}": Apos o cabeção
					 * do if "if(...)" não si deve utilizar ";"
					 */
					int elementoDoFechoLambda = 0;

					while (tabelaAFNDLambda[tabelaAFNDLambda[linha][coluna][elementoDoConjunto] /* Linha do elemento (estado)que foi inserido*/][colunaFechoLambda][elementoDoFechoLambda] != 999) {// Até o utimo elento valido
				...
				while (tabelaAFNDLambda[linha][colunaFechoLambda][elementoDoConjuntoFechoLambda] != 999) {// Loop em toda os elentos do conjunto fecho lambda
																										

					/*
					 * 2 Parte: Realiza a inserção que eu chamo de "Z" Para cada
					 * elemento pertecente tabelaAFNDLambda[ linha ][
					 * colunaFechoLambda ][ elementoDoConjuntoFechoLambda ]
					 * (conjunto fecho lambda da linha atual) va ate sua posição
					 * na tabela tabelaAFNDLambda e verifique e exite elemento
					 * para a transição atual(ou seja a coluna que é igual ao
					 * simbolo) si exite insere esse elemento (caso nao tenha
					 * sido inserido ainda) na tabela
					 * tabelaAFND[linha][coluna][...] em seguida realizar
					 * inserção em "L" desse elemento
					 */

					elementoDoConjunto = 0;
					while (tabelaAFNDLambda[ /* Elemento do fecho lambda */tabelaAFNDLambda[linha][colunaFechoLambda][elementoDoConjuntoFechoLambda]][coluna][elementoDoConjunto] != 999) {// Loop
																																															// em
																																															// todos
																																															// os
																																															// elementos
						...
						// Erro em estrutura de if sem chaves "{}": Apos o
						// cabeção do if "if(...)" não si deve utilizar ";"
						int elementoDoFechoLambda = 0;
		...
		/*
		 * Montagem da tabela AFD que contem "TamanhoDoAlfabeto + 1" colunas
		 * ondea primeira coluna sera inserido os novos conjuntos
		 * 
		 * Obs: Si o AFND tiver 'n' estados então o AFD pode terno maximo 2
		 * elevado n estados
		 */
		...
		tabelaAFD = new int[maximoDeEstadoAFD][tamanhoDoAlfabeto + 1][tamanhoMaxDosConjuntos];// "TamanhoDoAlfabeto + 1" pois temos uma nova coluna de conjunto "coluna 0"
		...
		while (tabelaAFD[linha][0][0] != 999) {
			/*
			 * Si o primeiro elemento do conjunto for 999 ele esta vazio//Loop
			 * em toda coluna 0
			 */

			for (int coluna = 1; coluna < tamanhoDoAlfabeto + 1; coluna++) {// Coluna inicia no 1 pois a coluna 0 pertence aos novos estados que serão inseridos em outra parte
																		

				int elementoDoNovoEstado = 0;

				while (tabelaAFD[linha][0][elementoDoNovoEstado] != 999) {// Loop em todos os elentos do conjunto
																		
					/*
					 * Para cada elemento do conjunto(Novo estado na coluna 0)
					 * na tabela AFD vai na tabela AFND e insere o conjuto da
					 * sua transiçao de acordo com a entrada (letra do alfabeto
					 * coluna )
					 */

					int elementoDaTabelaAFND = 0;

					while (tabelaAFND[/* linha */tabelaAFD[linha][0][elementoDoNovoEstado]][coluna - 1/*-1 Correção entre tabelas*/][elementoDaTabelaAFND] != 999) {// Até
																																										// o
																																										// ultimo
																																										// elemento
																																										// valido
						if (verificarExistenciaDoElementoNoCojunto(
								...
								linha,
								coluna,
								tabelaAFND[/* linha */tabelaAFD[linha][0][elementoDoNovoEstado]][coluna - 1][elementoDaTabelaAFND]) == false)
							inserirOrdenado(
									tabelaAFD,
									linha,
									coluna,
									tabelaAFND[/* linha */tabelaAFD[linha][0][elementoDoNovoEstado]][coluna - 1][elementoDaTabelaAFND]);
				...
				/*
				 * Verificar si o cojunto ja foi inseridona coluna '0' ou seja
				 * si é um novo conjuntoSi for um novo conjunto então inserir
				 */
		...
		BufferedWriter buffWrite = new BufferedWriter(
				new FileWriter(
						"/home/davidg/Documentos/ProgramasJava/ProgramasComEclip/Converte/src/saida1.txt"));
		...
		buffWrite.write(strBuilderCabecalho.toString(), 0,
				strBuilderCabecalho.length());// salvando cabeçalho no arquivo
		buffWrite.append("\n");
		...
		for (int i = 0; i < quantidadeEstados; i++) {// Linha(estado)
			...
			for (int j = 0; j < quantidadeDeColunasTabelaAFNDLambda - 1; j++) {// Coluna(Simbolo) -1 para nao salva linha fecho lambda no arquivo
				...
				for (int l = 0; l < quantidadeEstados; l++) {// Elemento do
																// conjunto
					...
					if (tabelaAFNDLambda[i][j][l] == 999) {
						...
						if (l == 0)// Si for o primeiro elemento do conjunto e
									// conjunto nao for vazio insira { antes
							strBuilderLinha.append('{');

						// Si for o ultimo elemento do conjunto nao inserir
						// virgula
		...
		/*
		 * Savando tabela AFND no arquivo
		 */
		...
		buffWrite.write(strBuilderCabecalhoAFND.toString(), 0,
				strBuilderCabecalhoAFND.length());// salvando cabeçalho no
													// arquivo
		buffWrite.append("\n");
		...
		for (int i = 0; i < quantidadeEstados; i++) {// Linha(estado)
			...
			for (int j = 0; j < tamanhoDoAlfabeto; j++) {// Coluna(Simbolo) -1
															// Para nao salva
															// linha fecho
															// lambda no arquivo
				for (int l = 0; l < quantidadeEstados; l++) {// Elemento do
		...
		buffWrite.write(strBuilderCabecalhoAFD.toString(), 0,
				strBuilderCabecalhoAFD.length());// salvando cabeçalho no
													// arquivo
		buffWrite.append("\n");
		...
		for (int i = 0; i < maximoDeEstadoAFD; i++) {// Linha(estado)
			...
			if (tabelaAFD[i][0][0] != 999) {// Si a linha represetar um conjuto
											// salve
				buffWrite.write(strBuilderLinha.toString(), 0,
						strBuilderLinha.length());
				buffWrite.append("\n");
	...
	public static Boolean comparaDoisConjunto(int[] conjunto1, int[] conjunto2) {
		 ...
		 * 
		 * Bloco e dividido em duas partes1 verifica si os dois conjunto possuem
		 * tamanhos iguais2 Verifica elemento a elemento si são iguais
		 * 
	...
	public static Boolean verificarExistenciaDoElementoNoCojunto(
			...
			int elemento) {
		/*
		 * Verifica si cunjunto ja possui o elemento que se deseja inserir Si o
		 * elemento não existe no conjunto retorna false
		 */
	...
	public static void inserirOrdenado(int[][][] tabelaEntrada,
		...
		while (inserir != true) {// Inserir no lugar do primeiro maior elemento
									// encontrado
		...
		if (posDeslocar != tabela[0][0].length - 1)// Ultimo elemento não
													// precisa deslocar
	...
	public static void fecholambdaRecursiva(int[][][] tabelaFNDLambda,
		 ...
		 * 
		 * Dado um estado(linha da tabela ) este metodo forma o conjunto lambda
		 * do mesmo. Ele percorrer recursivamente chamando o metodo
		 * "fecholambdaRecursiva" passando um elento do cunjunto por vez em loop
		 * formando uma arvore de recurção.
		 * 
		 * Em outras palavras, para cada elemento do "conjunto Transição lambda"
		 * da linha atual ou seja tabela [ estadoAtual ][ colunaLambda ][...] va
		 * até seu conjunto Transição lambda" até o um caso base ( tabela [
		 * estadoAtual ][ colunaLambda ][...] = vazio) voutando inserindo os
		 * elemento.
		 */

		int[][][] tabela = tabelaFNDLambda;

		if (tabela[estadoAtual][colunaLambda][0] == 999) {// Caso base cojunto
															// vazio
			if (verificarExistenciaDoElementoNoCojunto(tabela,
					estadoFechoLambda, colunaFechoLambda, estadoAtual) == false) // verifica si ja existir o elemento no vetor si nao inserir de forma ordenada
			...
			while (tabela[estadoAtual][colunaLambda][i] != 999) {// Analisa todo selementos do conjunto lambda
